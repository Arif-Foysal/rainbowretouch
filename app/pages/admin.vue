...
const saveCategory = async () => {
  ...
  if (categoryModalMode.value === 'create') {
    ;({ error } = await (supabase as any).from('service_categories').insert(payload))
  } else {
    ;({ error } = await (supabase as any).from('service_categories').update(payload).eq('id', categoryForm.id))
  }
...
const openEditFeatureModal = (service: ServiceItemRecord, feature: FeatureRecord) => {
...
const saveService = async () => {
  ...
  if (serviceModalMode.value === 'create') {
    ...
    ;({ error } = await (supabase as any).from('service_items').update(payload).eq('id', serviceForm.id))
  }
...
const saveFeature = async () => {
  if (!featureForm.label.trim()) {
    toast.add({ title: 'Validation error', description: 'Feature label is required.', color: 'error' })
    return
  }
  if (!featureForm.serviceId) {
    toast.add({ title: 'Validation error', description: 'Missing service reference.', color: 'error' })
    return
  }
  ...
  if (featureModalMode.value === 'create') {
    ;({ error } = await (supabase as any).from('service_item_features').insert(payload))
  } else {
    ;({ error } = await (supabase as any).from('service_item_features').update(payload).eq('id', featureForm.id))
  }

  if (error) {
    toast.add({ title: 'Error', description: 'Failed to save feature.', color: 'error' })
  } else {
    toast.add({
      title: featureModalMode.value === 'create' ? 'Feature created' : 'Feature updated',
...
const deleteFeature = async (feature: FeatureRecord) => {
  ...
  if (error) {
    toast.add({ title: 'Error', description: 'Failed to delete feature.', color: 'error' })
  } else {
    toast.add({ title: 'Feature deleted', description: 'Service features updated.', color: 'success' })
    await fetchServicesNavigation()
  }
}
...
const saveImage = async () => {
  ...
  if (imageModalMode.value === 'create' && !imageUploadFiles.value.length) {
    toast.add({ title: 'Validation error', description: 'Please select an image file.', color: 'error' })
    return
  }
  ...
  if (imageUploadFiles.value.length) {
    ...
    const { error: uploadError } = await supabase.storage.from(serviceImageBucket).upload(path, file, {
      cacheControl: '3600',
      upsert: false
    })
    ...
    const { data: publicData } = supabase.storage.from(serviceImageBucket).getPublicUrl(path)
    publicUrl = publicData?.publicUrl || ''
    storagePath = path
  }

  if (!publicUrl) {
    toast.add({ title: 'Error', description: 'Missing image URL after upload.', color: 'error' })
    isSavingImage.value = false
    return
  }
  ...
  const payload = {
    ...
    order_index: Number.isFinite(imageForm.order_index) ? imageForm.order_index : 1
  }

  let error
  if (imageModalMode.value === 'create') {
    ;({ error } = await (supabase as any).from('service_item_images').insert(payload))
  } else {
    ;({ error } = await (supabase as any).from('service_item_images').update(payload).eq('id', imageForm.id))
  }

  if (error) {
    toast.add({ title: 'Error', description: 'Failed to save image.', color: 'error' })
  } else {
    toast.add({
      title: imageModalMode.value === 'create' ? 'Image uploaded' : 'Image updated',
...
const deleteImage = async (image: ImageRecord) => {
  ...
  if (error) {
    toast.add({ title: 'Error', description: 'Failed to delete image.', color: 'error' })
  } else {
    toast.add({ title: 'Image deleted', description: 'Service images updated.', color: 'success' })
    await fetchServicesNavigation()
  }
}
...
const fetchServicesNavigation = async () => {
  ...
  const { data, error } = await supabase
    .from('service_categories')
    .select('id, label, description, icon, order_index, service_items(id, label, description, icon, href, order_index, service_item_features(id, label, icon, description, created_at), service_item_images(id, image_url, storage_path, alt, order_index, created_at))')
  ...
  if (error) {
    ...
    serviceCategories.value = typedData.map((category: any) => ({
      ...
      service_items: (category.service_items ?? []).map((item: any) => ({
        ...
        service_item_images: (item.service_item_images ?? []).map((image: any) => ({
          id: image.id,
        ...
        service_item_features: (item.service_item_features ?? []).map((feature: any) => ({
          id: feature.id,
          label: feature.label,
          icon: feature.icon,
...
<template>
  <div class="min-h-screen bg-background">
        ...
        <template #header>
          <div class="flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p>Select icons from <a href="https://icones.js.org/" target="_blank" class="text-primary underline">Icones</a></p>
            </div>
            <UButton
              label="Add Category"
              icon="i-lucide-plus"
                  ...
                  >
                    <div class="flex  items-start gap-3">
                      <div class="flex h-10 w-10 items-center justify-center rounded-full bg-primary/10 text-primary">
                        <UIcon :name="serviceItem.icon || 'i-lucide-sparkles'" class="h-4 w-4" />
                      </div>
                      <div>
                        <p class="font-medium text-highlighted">{{ serviceItem.label }}</p>
                        <p v-if="serviceItem.description" class="text-sm text-muted mt-1">
                          {{ serviceItem.description }}
                        </p>
                        <div class="mt-1 flex flex-wrap gap-4 text-xs text-muted">
                          <span>Order: {{ serviceItem.order_index }}</span>
                          <span v-if="serviceItem.href">URL: {{ serviceItem.href }}</span>
                        </div>
                      </div>
                    </div>

                    <div
                      v-if="Array.isArray(serviceItem.service_item_features) && serviceItem.service_item_features.length"
                      class="mt-3 rounded-xl border border-dashed border-default p-3 bg-muted/20 w-full"
                    >
                      <p class="text-xs font-semibold uppercase tracking-wide text-muted mb-2">Features</p>
                      <div class="space-y-2">
                        <div
                        ...
                        >
                          <div class="flex items-start gap-3">
                            <div class="flex h-8 w-8 items-center justify-center rounded-full bg-primary/10 text-primary">
                              <UIcon :name="feature.icon || 'i-lucide-star'" class="h-4 w-4" />
                            </div>
                            <div>
                              <p class="font-medium text-highlighted">{{ feature.label }}</p>
                              <p v-if="feature.description" class="text-xs text-muted mt-1">{{ feature.description }}</p>
                            </div>
                          </div>
                          <div class="flex items-center gap-2">
                            <UButton
                              ...
                              variant="ghost"
                              color="neutral"
                              @click="openEditFeatureModal(serviceItem, feature)"
                            />
                            <UButton
                              size="xs"
                              icon="i-lucide-trash"
                    ...
                    <div
                      v-if="Array.isArray(serviceItem.service_item_images) && serviceItem.service_item_images.length"
                      class="mt-3 rounded-xl border border-dashed border-default p-3 bg-muted/10 w-full"
                    >
                      <p class="text-xs font-semibold uppercase tracking-wide text-muted mb-2">Photos</p>
                      <div class="grid gap-3 grid-cols-1">
                        ...
                        >
                          ...
                          <div class="flex items-center justify-between px-3 py-2 text-xs text-muted">
                            <span class="truncate">{{ image.alt || 'No alt text' }}</span>
                            <span class="text-[11px]">Order: {{ image.order_index }}</span>
                          </div>
    ...
    <UModal
      v-model:open="isFeatureModalOpen"
      :ui="{ content: 'max-w-xl' }"
    >
      <template #header>
        <div>
          <p class="text-sm text-muted">{{ featureModalMode === 'create' ? 'Create feature' : 'Edit feature' }}</p>
          <p class="text-lg font-semibold text-highlighted">
            {{ featureModalMode === 'create' ? 'New Service Feature' : 'Update Service Feature' }}
          </p>
        </div>
      </template>
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          />
          <UButton
            :label="featureModalMode === 'create' ? 'Create feature' : 'Save changes'"
            color="primary"
    ...
    >
      <template #header>
        <div>
          <p class="text-sm text-muted">{{ imageModalMode === 'create' ? 'Upload photo' : 'Edit photo' }}</p>
          <p class="text-lg font-semibold text-highlighted">
            {{ imageModalMode === 'create' ? 'New Service Photo' : 'Update Service Photo' }}
          </p>
        </div>
      </template>
      ...
      <template #body>
        <div class="space-y-4">
          ...
          <UFormGroup label="Photo file" :required="imageModalMode === 'create'">
            <UFileUpload
      ...
      <template #footer>
        <div class="flex justify-end gap-3">
          ...
          />
          <UButton
            :label="imageModalMode === 'create' ? 'Upload photo' : 'Save changes'"
            color="primary"
