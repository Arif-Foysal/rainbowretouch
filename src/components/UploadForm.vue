<template>
      ...
      <form @submit.prevent="submitForm" class="space-y-6">
        ...
        <div class="grid gap-2">
          ...
          <input 
            ...
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 outline-none"
            required
          />
        </div>
        ...
        <div class="grid gap-2">
          ...
          <div class="relative">
            <select 
              ...
              class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 outline-none appearance-none bg-white"
              required
            >
              ...
              <option value="photo_manipulation">Photo Manipulation</option>
            </select>
        ...
        <div class="grid gap-2">
          ...
          <textarea 
            ...
            rows="4" 
            placeholder="Please Enter Notes / Project Brief"
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-indigo-600 focus:border-indigo-600 outline-none"
          ></textarea>
        </div>
        
        <!-- File Format Selection -->
        <div v-if="showFormats" class="grid gap-2">
          <label class="text-sm font-medium text-gray-700">Formats</label>
          <div class="flex flex-wrap gap-4">
            <label v-for="format in formats" :key="format" class="inline-flex items-center">
              <input 
                type="radio" 
                name="format" 
                :value="format" 
        ...
        <div class="w-full mb-5">
          <label 
            ...
            @drop.prevent="onDrop"
            :class="{ 'bg-gray-100 border-indigo-600': isDragging }"
          >
        ...
        <div v-if="files.length > 0" class="w-full grid gap-4 mb-4">
          <div 
          ...
          >
            <div class="flex items-center justify-between gap-2">
              <div class="flex items-center gap-2">
                <svg v-if="file.type === 'complete'" class="mx-auto" xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 40 40" fill="none">
                  <g id="File">
                ...
                <div class="grid gap-1">
                  <h4 class="text-gray-900 text-sm font-normal leading-snug">{{ file.name }}</h4>
                  <h5 class="text-gray-400 text-xs font-normal leading-4">
                    {{ file.type === 'complete' ? 'Upload complete' : `${file.remainingTime} remaining` }}
                  </h5>
                </div>
              </div>
              <div 
        ...
        <div v-if="showSubmitButton">
          <button 
            type="submit" 
            class="w-full bg-indigo-600 text-white py-2 px-4 rounded-lg hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition-colors"
          >
            Submit
          </button>
        </div>
  ...
  // Available formats
  const formats = ['JPG', 'PNG', 'PSD', 'TIFF', 'GIF']
  ...
  const processFiles = (fileList) => {
    for (let i = 0; i < fileList.length; i++) {
      ...
      if (!fileType.match(/image\/(jpeg|jpg|png)|application\/pdf/)) {
        alert(`File ${file.name} is not supported. Please upload PNG, JPG or PDF files.`)
        continue
      }
  ...
  const simulateUpload = (fileIndex) => {
    ...
    const interval = setInterval(() => {
      ...
      if (progress >= 100) {
        ...
        files.value[fileIndex].type = 'complete'
        files.value[fileIndex].remainingTime = ''
      } else {
        // Calculate remaining time
        const remainingSeconds = Math.floor(Math.random() * 30) + 1
        files.value[fileIndex].remainingTime = `${remainingSeconds} seconds remaining`
      }
  ...
  const submitForm = () => {
    ...
    const stillUploading = files.value.some(file => file.type === 'uploading')
